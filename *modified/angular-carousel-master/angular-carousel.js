...
.directive('rnCarouselControls', [function() {
  ...
  link: function(scope, element, attrs) {
    var backUp = 0;
    scope.showingNextSlide = false; //showingNextSlide is NOT showContinue
    scope.$parent.showshowhsow = 0;
    scope.prev = function() {
        if (scope.index > 0){
            if(backUp <= scope.index) {
                backUp = scope.index;
                scope.showingNextSlide = true;
            }
            scope.index--;
        }
    };
    ...
    scope.continue = function() {
        if (scope.index < scope.items.length-1) {
            backUp = scope.index;
            scope.index++;
            scope.$parent.showshowhsow = scope.index;
        }
    };
},
...
'<span class="rn-carousel-control rn-carousel-control-next" ng-show="showingNextSlide" ng-click="next()" ng-if="index < items.length - 1"></span>' +
'<span class="rn-carousel-control rn-carousel-control-continue" ng-hide="showingNextSlide" ng-if="index < items.length - 1"><button ng-click="continue()">Continue</button></span>' +
'</div>'
};
}]);
    ...
    .directive('rnCarousel', ['$swipe', '$window', '$document', '$parse', '$compile', '$rootScope', function($swipe, $window, $document, $parse, $compile, $rootScope) {
        ...
        var carouselId = 0,
            ...
            return {
                ...
                //tElement.children().attr("ng-if", "showshowhsow < $index");
                var indexFroHiding=0;
                angular.forEach(tElement.children(), function(value, key){
                     var a = angular.element(value);
                     a.attr("ng-hide","showshowhsow < "+indexFroHiding);
                     //a.text("{{showshowhsow}}")
                     indexFroHiding++
                });
                /*tElement.children().each(function( index ) {
                    console.log( index + ": " + this.setAttribute("test",index) );
                });*/
